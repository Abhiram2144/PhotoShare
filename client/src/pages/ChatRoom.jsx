...
const ChatRoom = () => {
  ...
  const encryptImageAESGCM = async (buffer) => {
    const rawKey = new TextEncoder().encode("12345678901234567890123456789012"); // Keep this safe
  ...
  const handleSendImage = async ({ file, caption }) => {
    ...
    const { encryptedBuffer, nonce } = await encryptImageAESGCM(buffer);

    const encryptedBlob = new Blob([encryptedBuffer]);
    const formData = new FormData();
    formData.append("chatId", chatId);
    formData.append("image", encryptedBlob);
